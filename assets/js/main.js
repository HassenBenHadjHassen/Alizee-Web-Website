...
(function ($) {
  ...
  // Persist and propagate ad tracking params across navigation
  (function () {
    var keys = [
      "gclid",
      "gbraid",
    ...
    function mergeParamsIntoUrl(href, persistedParams) {
      var url = new URL(href, window.location.href);
      if (!persistedParams) return url.toString();
      var add = new URLSearchParams(persistedParams.slice(1));
      // Do not overwrite if already present on the link
    ...
    function propagateParamsOnLinks() {
      var persisted = buildPersistedQuery();
      if (!persisted) return;
      ...
      links.forEach(function (a) {
        ...
        var updated = mergeParamsIntoUrl(href, persisted);
        a.setAttribute("href", updated);
      });
    }

    function injectParamsIntoContactForm() {
    ...
    document.addEventListener("DOMContentLoaded", function () {
      propagateParamsOnLinks();
      injectParamsIntoContactForm();
    });
  })();
})(jQuery);
